        ...
        try {
            ...
            if (savedTasks) {
                ...
                if (Array.isArray(parsed)) {
                    ...
                    throw new Error('保存されたタスクデータの形式が無効です');
                }
            }
        } catch (error) {
            ...
            // エラー通知
            if (window.uiManager) {
                uiManager.showNotification('タスクの読み込み中にエラーが発生しました。データをリセットしました。', 'warning');
            }
        }
    ...
    async executeTask(taskId) {
        try {
            ...
            if (!window.processorSystem) {
                throw new Error('プロセッサーシステムが利用できません');
            }

            // AIへの指示がある場合は追加
            const instructions = task.aiInstructions 
                ? `\n\nAIへの追加指示:\n${task.aiInstructions}`
                : '';

            // タスクの実行プランを生成
            const plan = await processorSystem.generatePlan(
                `タスク: ${task.title}\n説明: ${task.description}${instructions}`
            );
            ...
            // プランを実行
            await processorSystem.executePlan(
            ...
            // エラー通知を表示
            if (window.uiManager) {
                uiManager.showNotification(`タスクの実行に失敗しました: ${error.message}`, 'error');
            }
