...
class ApiService {
    ...
    async sendRequest(params) {
        ...
        try {
            ...
            // API特有のエラーメッセージをより分かりやすく変換
            if (error.message.includes('429')) {
                throw new Error('APIリクエストの制限に達しました。しばらく待ってから再試行してください。');
            } else if (error.message.includes('401') || error.message.includes('403')) {
                throw new Error('APIキーが無効です。設定から正しいAPIキーを入力してください。');
            } else if (error.message.includes('500')) {
                throw new Error('APIサーバーでエラーが発生しました。しばらく待ってから再試行してください。');
            } else if (error.message.includes('timeout')) {
                throw new Error('APIリクエストがタイムアウトしました。ネットワーク接続を確認して再試行してください。');
            }
            throw error;
        }
    }
