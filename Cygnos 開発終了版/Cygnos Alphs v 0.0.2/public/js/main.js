...
window.addEventListener('error', function(event) {
    ...
    if (window.uiManager) {
        window.uiManager.showNotification(`エラーが発生しました: ${event.error.message}`, 'error');
    }
});
...
window.addEventListener('unhandledrejection', function(event) {
    ...
    if (window.uiManager) {
        window.uiManager.showNotification(`非同期処理エラー: ${event.reason.message || '不明なエラー'}`, 'error');
    }
});

// DOMContentLoadedイベントリスナーの改善
document.addEventListener('DOMContentLoaded', async () => {
    try {
        // 各コンポーネントの初期化を順番に実行
        if (!window.settingsManager) {
            throw new Error('設定マネージャーの初期化に失敗しました');
        }
        
        if (!window.memorySystem) {
            throw new Error('メモリーシステムの初期化に失敗しました');
        }
        
        if (!window.processorSystem) {
            throw new Error('プロセッサーシステムの初期化に失敗しました');
        }
        
        if (!window.uiManager) {
            throw new Error('UIマネージャーの初期化に失敗しました');
        }
        
        // APIキーの確認
        if (!settingsManager.hasRequiredApiKeys()) {
            uiManager.showNotification('APIキーが設定されていません。設定から入力してください。', 'warning');
